from hermes_c import hr
from database.connection import get_session
from database.crud import insert_fa_data, insert_ta_data
import logging
...
async def fa_data(ticker: str):
    data = await hr.fa_features.get_fundamentels(symbol=ticker)
    return data

async def ta_data(symbol: str):
    ...
    with get_session() as session:
        try:
            """for symbol in SYMBOLS:
                try:
                    data = await ta_data(symbol=symbol)
            ...
            for ticker in TICKERS:
                try:
                    data = await fa_data(ticker=ticker)
