...
JOBS = {
    "ingest": lambda: __import__(
        "sentinel.ingestion.ingestion_pipeline", fromlist=["run_pipeline"]
    ).run_pipeline(),
    "features": lambda: __import__(
        "sentinel.ingestion.feature_engineer", fromlist=["main"]
    ).main(),
    "retrain": lambda: __import__(
        "sentinel.ml.train_pipeline", fromlist=["run_pipeline_with_metrics"]
    ).run_pipeline_with_metrics(),
    "drift": lambda: __import__("sentinel.mlops.drift_detector", fromlist=["run_drift_check"]),
}
